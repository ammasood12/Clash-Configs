...
config_after_install() {
    echo -e "${yellow}For security reasons, you need to forcefully change the port and account password after installation/update is complete${plain}"
    read -p "Confirm whether to continue. If you select n, you will skip the port and account password settings.[y/n]": config_confirm
    if [[ x"${config_confirm}" == x"y" || x"${config_confirm}" == x"Y" ]]; then
        ...
        if [[ ! -f "/etc/x-ui/x-ui.db" ]]; then
            ...
            echo -e "It is detected that you are a new installation. For security reasons, a random user and port have been automatically generated for you.:"
            echo -e "###############################################"
            echo -e "${green}Panel login user name:${usernameTemp}${plain}"
            echo -e "${green}Panel login user password:${passwordTemp}${plain}"
            echo -e "${red}Panel login port:${portTemp}${plain}"
            echo -e "###############################################"
            echo -e "${red}If you forget the panel login information, you can enter x-ui after the installation is complete and enter option 7 to view the panel login information.${plain}"
        else
            echo -e "${red}This is a version upgrade. The previous settings are retained and the login method remains unchanged. You can enter x-ui followed by the number 7 to view the panel login information.${plain}"
        fi
    fi
}
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}Failed to detect the x-ui version. This may be due to exceeding the Github API limit. Please try again later or manually specify the x-ui version to install.${plain}"
            exit 1
        fi
        echo -e "The latest version of x-ui has been detected：${last_version}，Start the installation"
    ...
    config_after_install
    #echo -e "For a fresh installation, the default web page port is ${green}54321${plain}, and the default username and password are ${green}admin${plain}"
	#echo -e "Please ensure that this port is not occupied by other programs, ${yellow} and that port 54321 is open. ${plain}"
	#echo -e "To change port 54321 to another port, enter the x-ui command. Also, ensure that the port you change to is open."
	#echo -e ""
	#echo -e "If you are updating the panel, access the panel as before."
	#echo -e ""
    ...
    echo -e "----------------------------------------------"
    echo -e "x-ui 			- Display the management menu (more features)"
	echo -e "x-ui start 	- Start the x-ui panel"
	echo -e "x-ui stop 		- Stop the x-ui panel"
	echo -e "x-ui restart 	- Restart the x-ui panel"
	echo -e "x-ui status 	- Check the x-ui status"
	echo -e "x-ui enable 	- Set x-ui to start automatically at startup"
	echo -e "x-ui disable 	- Disable x-ui from starting automatically at startup"
	echo -e "x-ui log 		- Check the x-ui log"
	echo -e "x-ui v2-ui 	- Migrate the local v2-ui account data to x-ui"
